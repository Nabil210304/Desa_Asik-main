...
<div class="max-w-7xl mx-auto mb-10 mt-5 bg-white p-8 rounded-lg shadow-lg">
    ...
    <a href="/admin_berita_saya/{{ Auth::user()->id_desa }}" class="inline-block bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded mt-4">
        Kembali
    </a>
</div>
...
document.getElementById('referensi-wrapper').addEventListener('click', function (e) {
    if (e.target.closest('.tambah')) {
        ...
        newField.className = 'flex gap-2 items-center mt-2';
        newField.innerHTML = `
            <input type="text" name="referensi[]" class="w-full border p-2 rounded" required />
            <button type="button" class="hapus bg-red-600 text-white px-3 rounded text-xl"><i class="fas fa-minus"></i></button>
        `;
