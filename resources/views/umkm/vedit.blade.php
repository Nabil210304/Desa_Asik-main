...
<div class="max-w-7xl mx-auto mb-10 mt-5 bg-white p-8 rounded-lg shadow-lg">
    ...
    <form method="POST" action="/edit_umkm_admin/{{$umkm->id_umkm}}" enctype="multipart/form-data" id="form-desa" class="space-y-4">
        @csrf
        @method('PUT')
   ...
   {{-- Input untuk menyimpan path foto yang sudah ada --}}
    <div id="existing-photos-container">
        @foreach($fotoProduk as $foto)
        <div class="flex items-center gap-2 mb-2 existing-photo-row">
            <input type="hidden" name="existing_foto_produk[]" value="{{ $foto }}">
            <span class="flex-1 px-3 py-2 text-gray-700 bg-gray-100 border rounded">
                {{ basename($foto) }}
            </span>
            <button type="button" class="hapus-foto-lama bg-red-600 text-white px-3 py-2 rounded">-</button>
        </div>
        @endforeach
    </div>

    {{-- Tambah input baru jika ingin upload foto produk tambahan --}}
    ...
    @forelse ($sosmedList as $jenis => $link)
        @if (!empty($link))
            <div class="flex gap-2 items-center">
                ...
                <input type="text" name="link_sosmed[]" placeholder="Masukkan link sosial media"
                       class="border rounded p-2 flex-1" value="{{ $link }}">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('existing-photos-container').addEventListener('click', function(e) {
            if (e.target && e.target.classList.contains('hapus-foto-lama')) {
                e.target.parentElement.remove();
            }
        });

         document.getElementById('tambah-foto-baru').addEventListener('click', function() {
        const container = document.getElementById('new-photos-container');
        const div = document.createElement('div');
        div.className = 'flex items-center gap-2 mb-2';
        div.innerHTML = `
            <input type="file" name="new_foto_produk[]" class="w-full border rounded p-2" accept=".jpg,.jpeg,.png">
            <button type="button" class="hapus-baris-baru bg-red-600 text-white px-3 py-2 rounded">-</button>
        `;
        container.appendChild(div);
    });

    // Hapus baris input file baru
    document.querySelector('body').addEventListener('click', function(e) {
        if (e.target && e.target.classList.contains('hapus-baris-baru')) {
            e.target.parentElement.remove();
        }
    });
